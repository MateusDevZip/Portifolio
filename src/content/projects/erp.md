---
title: Painel de Gestão ERP
year: 2023
category: Gestão e Automação
description: Um dashboard robusto desenvolvido com Vue.js e Python (Django) para gerenciar estoques, emissões de NFs e relatórios financeiros em tempo real.
color: "#a855f7"
technologies: ["Vue.js", "Django", "PostgreSQL", "Redis"]
link: https://erp-example.com
---

## Visão Geral

Sistema ERP completo e integrado para gestão de operações empresariais. Desenvolvido como uma solução full-stack com frontend dinâmico e backend robusto.

## Funcionalidades Principais

- **Gestão de Estoques:** Controle em tempo real de entradas e saídas
- **Emissão de NFs:** Integração com autoridades fiscais
- **Relatórios Financeiros:** Dashboards customizáveis
- **Controle de Vendas:** Pipeline e acompanhamento de pedidos

## Arquitetura

O projeto segue padrões de arquitetura limpa com separação clara entre camadas. O backend em Django fornece uma API REST robusta enquanto o Vue.js oferece uma interface reativa e responsiva.

## Tecnologias Utilizadas

- **Frontend:** Vue.js 3, Vuex, Axios
- **Backend:** Django, Django REST Framework
- **Database:** PostgreSQL
- **Cache:** Redis
- **Autenticação:** JWT
- **Infra:** Docker, AWS EC2

## Impacto

- Redução de 60% no tempo de processamento manual
- Aumento de 80% na acurácia de dados
- Dashboard em tempo real melhorou decisões
- Escalou para 5000+ usuários simultâneos
